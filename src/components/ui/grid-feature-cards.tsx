...
function GridPattern({
	...
	squares,
	...props
}: React.ComponentProps<'svg'> & { width: number; height: number; x?: string | number; y?: string | number; squares?: number[][] }) {
	...
	return (
		<svg aria-hidden="true" {...props}>
			...
			{squares && (
				<svg x={x ?? 0} y={y ?? 0} className="overflow-visible">
					{squares.map(([squareX, squareY], index) => (
						<rect strokeWidth="0" key={index} width={width + 1} height={height + 1} x={(squareX ?? 0) * width} y={(squareY ?? 0) * height} />
					))}
				</svg>
			)}
		</svg>
